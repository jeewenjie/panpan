...
def simple_fx(X_file,batch_size,verbose):
    predictions = model.predict(X_file,batch_size,verbose)
    top_1 = np.array(LABELS)[np.argsort(-predictions, axis=1)[:, :1]]
    predicted_labels = [' '.join(list(x)) for x in top_1]
    #print('The word predicted is "%s".' %predicted_labels[0])
...
if __name__ == '__main__':
    ...
    test = pd.read_csv("/media/audiodsp/ExternalVolume/Google_speech_command/test_wav_reference.csv")
    ...
    # Load file
    #file_1 = test.iloc[7]
    #X_file = prepare_data(file_1,config,'/media/audiodsp/ExternalVolume/Google_speech_command/Real_time_materials/test_wav/')
    ...
    with Pool(2) as p:
        s = p.apply_async(simple_fx,(X_file,1,1))
        print(s)
