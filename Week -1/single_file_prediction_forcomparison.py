...
def prepare_data(df, config, file_path):
...
# Load file
file_1 = test.iloc[1]
X_file = prepare_data(file_1,config,'/home/audiodsp/Desktop/Weekly Reports/Week -1/newoutput.wav')
...
print(toc-tic)
prediction = top_1 = np.array(LABELS)[np.argsort(-predictions, axis=1)[:, :1]]
predicted_labels = [' '.join(list(x)) for x in top_1]
